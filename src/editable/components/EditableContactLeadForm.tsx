...
export function EditableContactLeadForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 rounded-sm border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] p-6 md:p-8">
      ...
      {message ? (
        <div className={`mt-5 flex items-start gap-3 rounded-sm px-4 py-3 text-sm font-semibold ${status === 'success' ? 'bg-[var(--slot4-accent-soft)] text-[var(--slot4-accent)]' : 'bg-red-50 text-red-700'}`}>
      ...
      <button
        ...
        className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--slot4-accent-fill)] px-6 text-sm font-semibold uppercase tracking-[0.18em] text-[var(--slot4-on-accent)] shadow-[0_8px_24px_rgba(198,242,78,0.22)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-70"
      >
